...
class Store_items extends MX_Controller 
{
    ...
    function view($update_id)
    {
        ...
        if($num_rows>0){
              //we have one galler picture this item
            $data['use_angularjs']=TRUE;

            //build an array of all gallery picture
            $count=0;
    ...
    function _get_sub_cat_id($update_id)
    {
        ...
        //http://localhost/rnshop/music/instruments/Fender-Guitars
    ...
    function _get_best_sub_cat_id($update_id)
    {
        ...
        //how many sub cats does  this item appear
        ...
        if($num_sub_cats_for_item==1)
        {
            ...
            //we have more than on sub cat START
            ...
            //which  array key is paired with the highted valued
            $sub_cat_id=$this->get_best_array_key($num_items_count);
            return $sub_cat_id;
            //we have more than on sub cat END
        }
    }
...
function item_check($str)
{
    ...
    if ($num_rows>0)
    {
        $this->form_validation->set_message('item_check', 'The item title that you submitted not avilable');
        return FALSE;
    }
    else
    {
