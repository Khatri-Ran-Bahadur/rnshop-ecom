...
function test_destroy()
{
    ...
    echo "The cookie has been Destroy";
    echo anchor('site_cookies/test', '<br>Attempt to get The Cookie');
     echo "<hr>";
    echo anchor('site_cookies/test_set', 'Set The Cookie');
}
...
function _attempt_get_user_id()
{
    ...
    //check to see if the user has valid  cookie and if so, figure out the user_id from the cookie
    $this->load->module('site_settings');
    $cookie_name=$this->site_settings->_get_cookie_name(); 
...
function _destroy_cookie()
{
    ...
    {
        $cookie_code=$_COOKIE[$cookie_name];
        $sql = "delete from site_cookies where cookie_code=?";
        $this->db->query($sql, array($cookie_code));
    }
