...
function article(){
  ...
  if($page_url==''){
    redirect("templates/page_not_found");
  }
...
function _draw_feed_hp($is_mobile=FALSE)
{
  ...
  $this->load->view($view_file, $data);
    ...
    function delete_image($update_id)
    {
        ...
        $small_picure=str_replace('.', '_thumb.', $picture);
        $small_pic_path='./blog_pics/'.$small_picure;
...
function test()
{
  ...
  //convert bck into unix timestamp
...
function delete($update_id)
{
  if(!is_numeric($update_id)){
   ...
   if($submit=='Cancel')
   {
    ...
    $flash_msg="The page was successfull deleted.";
    $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
    $this->session->set_flashdata('item', $value);
...
if($submit=="Cancel"){
    ...
    if($submit=="Submit"){
        ...
        $this->form_validation->set_rules('page_title','Blog Entry Title', 'required|max_length[255]');
        ...
        if($this->form_validation->run()==TRUE){
            ...
            if(is_numeric($update_id)){
                ...
                $flash_msg="The blog entry details was successfull updated.";
                $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('blog/create/'.$update_id);
            } else{
                $this->_insert($data);
                $update_id= $this->get_max(); //get the ID of a new page

                $flash_msg="The blog entry was successfull created.";
                $value ='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('blog/create/'.$update_id);
            }
     ...
     if(!is_numeric($update_id)){
        $data['headline']="Created New Blog";
    } else{
