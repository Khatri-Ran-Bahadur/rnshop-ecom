...
function submit_test()
{
    ...
    if(($on_test_mode==FALSE) OR (!is_numeric($num_orders))){
       ...
       //simulate order creation
        ...
        $this->store_orders->_auto_generate_orders($paypal_id, $customer_session_id);
...
if (strcmp ($res, "VERIFIED") == 0) {
  ...
  // The IPN is verified, process it
    ...
    $max_id=$this->get_max();
    $this->load->module('store_orders');
    $this->store_orders->_auto_generate_orders($max_id, $customer_session_id);
...
{
    return TRUE;//set to this FALSE if we are live
}
...
function _draw_checkout_btn($query)
{
    ...
    if($on_test_mode==TRUE){
        $data['form_location']='http://www.sandbox.paypal.com/cgi-bin/webscr';
    }else{
        $data['form_location']='http://www.paypal.com/cgi-bin/webscr';   
    }
