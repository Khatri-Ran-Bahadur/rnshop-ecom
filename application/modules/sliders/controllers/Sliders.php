...
{ 
    ...
    if($num_rows_ads>0){
        //we need to draw slider on this page
...
function delete($update_id)
{
  if(!is_numeric($update_id)){
   ...
   if($submit=='Cancel')
   {
    ...
    $flash_msg="The sliders was successfull deleted.";
...
function deleteconf($update_id)
{
   ...
   $data['headline']="Delete Entire Sliders";
...
{
    //draw the slides blocks that are on the homepage
...
function view($update_id)
{
    ...
    $this->load->module('custom_pagination');
        ...
        //fetch the sliders details
    $data=$this->fetch_data_from_db($update_id);

        //count the items that belong to this sliders
    $use_limit=FALSE;
 ...
 function create()
    {
        ...
        //process The form
            ...
            $this->form_validation->set_rules('slider_title','Slide Title', 'required|max_length[240]');
            ...
            //get the variable
                ...
                if(is_numeric($update_id)){
                //update the data sliders
                    $this->_update($update_id, $data);
                    $flash_msg="The sliders Details were successfully updated.";
                    $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                    $this->session->set_flashdata('item', $value);
                    redirect('sliders/create/'.$update_id);
                }else{
                //insert the new sliders

                $this->_insert($data);

                $update_id=$this->get_max();//get the ID of the new Homepage Slide                
                $flash_msg="The sliders was successfully Created.";
                $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->flashdata('item', $value);
                redirect('sliders/create/'.$update_id);
            }
        }
    }
    ...
    if(!is_numeric($update_id)){
        $data['headline']="Create New Homepage Sliders";
