...
$create_msg_url=base_url()."yourmessages/create"; ?><p style="margin-top: 30px;">
...
<table class="table table-striped table-bordered">
	...
	<tbody>
		...
		foreach ($query->result() as $row) {
			...
			$data_sent=$this->timedate->get_nice_date($row->date_created, 'mini');

			if($row->sent_by==0){
				$sent_by=$team_name;
			}else{
				$sent_by=$this->store_accounts->_get_customer__name($row->sent_by, $customer_data);
			}
