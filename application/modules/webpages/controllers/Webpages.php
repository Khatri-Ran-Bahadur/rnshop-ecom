...
if($submit=="Cancel"){
    ...
    if($submit=="Submit"){
        ...
        $this->form_validation->set_rules('page_title','Page Title', 'required|max_length[255]');
        ...
        if($this->form_validation->run()==TRUE){
            ...
            if(is_numeric($update_id)){
              ...
              if($update_id<3){
                ...
                $flash_msg="The webpage details was successfull updated.";
                $value='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('webpages/create/'.$update_id);
            } else{
                ...
                $flash_msg="The webpage details was successfull created.";
                $value ='<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('webpages/create/'.$update_id);
            }
    ...
    if((is_numeric($update_id)) && ($submit!="Submit")){
     ...
     if(!is_numeric($update_id)){
        $data['headline']="Created New Webpages";
    } else{
        $data['headline']="Update Webpage Details";
    }
