...
function test3()
{
    ...
    echo "The session vriable was unset ";
...
function  _in_you_go($user_id, $login_type)
{
    ...
    //NOTE: Log in type short term or Long term
...
function  _attempt_cart_divert($user_id)
{
    ...
    if($num_rows>0)
    {
        //there are records that need correct
...
function submit()
{
    ...
    if($submit=="Submit"){
            //process The form
                ...
                $this->form_validation->set_rules('email','Email', 'required|valid_email|max_length[160]');
                $this->form_validation->set_rules('pword','Password', 'required|min_length[8]|max_length[35]');
                $this->form_validation->set_rules('repeat_pword','Repeat Password', 'required|matches[pword]');

                if($this->form_validation->run()==TRUE){
                $query=$this->_process_create_acount();
...
function _process_create_acount() 
{
...
function username_check($str)
{
    ...
    $result=$this->site_security->_varify_hash($pword, $pword_on_table);
