...
{
    //NOTE:notifies customers when an order status has been updated
 ...
 function submit_order_status()
    {
        ...
        if($submit=='Cancel'){
            //get the current orders status for this order
            $query=$this->get_where($update_id);
            foreach ($query->result() as $row) {
                $order_status=$row->order_status;
            }
            ...
            $flash_msg="The order status was successfull updated.";
...
function view()
{
    ...
    //fetch the contents of the cart/basket
    $table='store_shoppertrack';
    $data['query_cc']=$this->cart->_fetch_carts_contents($session_id, $data['shopper_id'], $table); 
...
{
    //find out the total amount of in from this order
...
function _get_shopper_id($customer_session_id)
{
    ...
    $query=$this->store_basket->get_where_custom($customer_session_id);
    foreach ($query->reault() as $row) {
        $shopper_id=$row->shopper_id;
    }
    if(!isset($shopper_id))
    {
...
function _auto_generate_orders($max_id, $customer_session_id)
{
    ...
    $data['date_created']=time();
    $data['paypal_id']=$max_id;
    $data['session_id']=$customer_session_id;
    $data['opened']=0;
    $data['order_status']=0;
    $data['shoper_id']=$this->_get_shopper_id($customer_session_id);
