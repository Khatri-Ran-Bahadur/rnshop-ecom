...
class Store_accounts extends MX_Controller 
{
    ...
    function _make_sure_delete_allowed($update_id)
    {
        ...
        if($num_rows>0)
        {
            ...
            $mysql_query="select * from store_supporetrack where shopper_id=$update_id";
            $query= $this->_custom_query($mysql_query);
            $num_rows=$query->num_rows();
                if($num_rows>0){
                return FALSE;//shopper must have a purches
            }
        }
    ...
    function delete($update_id)
    {
      if(!is_numeric($update_id)){
       ...
       if($submit=='Cancel')
       {
        ...
        $flash_msg="The store account entry was successfull deleted.";
